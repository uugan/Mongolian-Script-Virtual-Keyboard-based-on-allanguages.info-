 ...
 *  $HeadURL: https://svn.debugger.ru/repos/jslibs/BrowserExtensions/tags/BrowserExtensions.027/helpers.js $
...
function () {
    ...
    var z = function (i) {
        ...
        var l = null,
            ...
            c = true,
            X = true;
        if (!i.currentTarget || !(l = i.currentTarget[x.UEID]) || !(O = e[l].handler[i.type])) return;
        try {
            for (var S = 0, k = O.length; S < k; S++) if (isFunction(O[S])) X = X && !(false === O[S].call(i.currentTarget, i));
        } catch (err) {
            setTimeout(function () {
                throw new Error("Event handler for [" + i.type + "] has failed with exception: \"" + err.message + "\"");
            }, 10)
        }
        return X
    };
                ...
                for (var _ in o.handler) {
                    ...
                    try {
                        if (Q.removeEventListener) {
                            Q.removeEventListener(_, o.rootEHCaller ? o.rootEHCaller : z, false);
                        } else if (Q.detachEvent) {
                            Q.detachEvent('on' + _, o.rootEHCaller ? o.rootEHCaller : z);
                        }
                    } catch (i) {};
    ...
    C.addEventListener = function (i, l, o) {
        ...
        if (!_.handler.hasOwnProperty(l)) {
            ...
            if (i.addEventListener) {
                i.addEventListener(l, z, false);
            } else if (i.attachEvent) {
                v.rootEHCaller = function (I) {
                    I.currentTarget = _.node;
                    var c = z(I);
                    I.currentTarget = null;
                    return c
                };
                i.attachEvent('on' + l, v.rootEHCaller);
            }
        } else {
            v = _.handler[l]
        }
        ...
        if (_ && (v = _.handler[l])) {
            ...
            if (0 == v.length) {
                ...
                if (i.removeEventListener) {
                    i.removeEventListener(l, v.rootEHCaller ? v.rootEHCaller : z, false);
                } else if (i.detachEvent) {
                    i.detachEvent('on' + l, v.rootEHCaller ? v.rootEHCaller : z);
                }
            }
            return true
        }
