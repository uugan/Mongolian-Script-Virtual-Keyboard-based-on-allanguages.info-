/**
 * $Id: vk.inc.js 684 2010-04-18 17:02:17Z wingedfox $
 *
 ...
 * @version $Rev: 684 $
 * @lastchange $Author: wingedfox $ $Date: 2010-04-18 21:02:17 +0400 (Вск, 18 Апр 2010) $
 */
var VirtualKeyboardLayout = function (i) {
    ...
    var q = function (y, Y) {
        if (c[0] == Y) {
            return Y + ":[" + addcslashes(y.map(function (P) {
                if (!P.match(/^\[.+\]$/)) return P.charCodeAt(0);
                else
                return P
            }).join(',')) + "]"
    ...
    I.serialize = function (y) {
        ...
        var U = [],
            ...
            A = [],
            d = [],
            D = $i_asc = $i_aac = $i_asac = $i_acc = $i_ascc = 0;
        for (var f in _) {
            var F = _[f],
                g = F['normal'],
                G = F['shift'],
